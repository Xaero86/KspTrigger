...
namespace KspTrigger
{
    public class TriggerConfig
    {
        ...
        public TriggerConfig(string name="Default")
        {
            Name = name;
        }
        
        private const string KEY_NB_TIMER   = "nbTimers";
        private const string KEY_PREF_TIMER = "timer";
        private const string KEY_NB_TRIG    = "nbTriggers";
        private const string KEY_PREF_TRIG  = "trigger";
            ...
            try {
                ...
                if (dataFound)
                {
                    for (int i = 0; i < nbItem; i++)
                    {
                        ...
                        if (newTrigger != null)
                        {
                            newTrigger.OnLoad(childNode, triggerConfig);
        ...
        public void LoadPersistentData()
        {
            try {
                if ((_triggers != null) && (_triggers.Count > 0))
                {
                    ...
                    {
                        trigger.LoadPersistentData(this);
                    }
                }
            } catch (Exception e) {
                Debug.LogError(Utils.DEBUG_PREFIX + e.Message);
        ...
        {
            AbsTimer instance;
            if (_timers.TryGetValue(name, out instance))
            {
    ...
    public class Trigger
    {
        ...
        public void LoadPersistentData(TriggerConfig triggerConfig)
        {
            ...
            if (_event != null)
            {
                _event.LoadPersistentData(triggerConfig);
            }
            // Condition
            _conditions.LoadPersistentData(triggerConfig);
            // Actions
            _actions.LoadPersistentData(triggerConfig);
        }
